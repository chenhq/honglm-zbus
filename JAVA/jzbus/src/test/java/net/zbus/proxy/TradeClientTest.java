...
import net.zbus.proxy.trade.TradeClientFactory;
...
public class TradeClientTest{ 
	public static void main(String[] args) throws Exception {
		...
		TradeClientFactory factory = new TradeClientFactory(config);
		
		PoolConfig poolCfg = new PoolConfig(); 
		poolCfg.setMaxActive(2); 
		Pool<TradeClient> pool = new Pool<TradeClient>(factory, poolCfg);
		
		TradeClient client = pool.borrowResource();
