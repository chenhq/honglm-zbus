...
char* gbk_to_utf8(const char* gbk_str){ 
	int n = MultiByteToWideChar(CP_ACP, 0, gbk_str, -1, NULL, 0);
	WCHAR* w_gbk = new WCHAR[n];
	MultiByteToWideChar(CP_ACP, 0, gbk_str, -1, w_gbk, n);
	n = WideCharToMultiByte(CP_UTF8, 0, w_gbk, -1, NULL, 0, NULL, NULL);
	char* utf = new char[n];
	WideCharToMultiByte(CP_UTF8, 0, w_gbk, -1, utf, n, NULL, NULL);
	delete[] w_gbk;   
	return utf;
}

char* utf8_to_gbk(char* utf_str){
	int len = MultiByteToWideChar(CP_UTF8, 0, utf_str, -1, NULL, 0);
	unsigned short * w_gbk = new unsigned short[len + 1];
	memset(w_gbk, 0, len * 2 + 2);
	MultiByteToWideChar(CP_UTF8, 0, utf_str, -1, (LPWSTR)w_gbk, len);

	len = WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)w_gbk, -1, NULL, 0, NULL, NULL);
	char *gbk= new char[len + 1];
	memset(gbk, 0, len + 1);
	WideCharToMultiByte(CP_ACP,0, (LPCWSTR)w_gbk, -1, gbk, len, NULL, NULL);
	delete[]w_gbk;
	return gbk;
}
