...
thread_recv(void* args){
	...
	while(1){  
		...
		if(zmsg_frame_size(request) % 2 == 0){
			...
			int rc = zbuswrk_send(zbus_conn, worker, reply);
		...
		if(g_kcbp_cfg->debug)
			zlog("KCXP SynCall BEGIN....\n");

		rc = KCBPCLI_CallProgramAndCommit(kcbp,funcid); 
		
		if(g_kcbp_cfg->debug)
			zlog("KCXP SynCall END (%d)\n", rc);
...
int main(int argc, char* argv[]){ 
	...
	char* brokers = zstrdup(g_kcbp_cfg->brokers);
	...
	int thread_count = list_size(broker_list)* g_kcbp_cfg->worker_threads;
	...
	while(1){
		...
		for(int i=0; i<g_kcbp_cfg->worker_threads; i++){
			threads[i] = zthread_new(thread_recv, zstrdup(broker)); 
		}  
		zfree(broker);
	} 
